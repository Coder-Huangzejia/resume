...
export function simpleDate(date: MomentInput): string | MomentInput | null | undefined {
...
export function simpleMonth(date: MomentInput): string | MomentInput | null | undefined {
    if (date === undefined || date === null || date === '') {
        return date;
    } else {
        return moment(transitionTimeZone({ date: date })).format('YYYYMM');
    }
}
...
export function simpleTime(date: MomentInput): string | MomentInput {
    if (date === undefined || date === null || date === '') {
        ...
        return moment(transitionTimeZone({ date: date })).format('YYYY-MM-DD HH:mm:ss');
    }
}

export function format(date: MomentInput, formater: string): string | MomentInput | null | undefined {
    if (date === undefined || date === null || date === '') {
        return date;
    } else {
...
export const quickPickTimeRange = {
    ...
    最近三天: [ moment().subtract(2, 'days').startOf('day'), moment().endOf('day') ],
...
export const quickFuturePickTimeRange = {
    ...
    未来一周: [ moment().add(1, 'days').startOf('day'), moment().add(8, 'days').endOf('day') ],
    未来一个月: [ moment().add(1, 'days').startOf('day'), moment().add(1, 'months').endOf('day') ],
    未来三个月: [ moment().add(1, 'days').startOf('day'), moment().add(3, 'months').endOf('day') ],
    未来半年: [ moment().add(1, 'days').startOf('day'), moment().add(6, 'months').endOf('day') ],
    未来一年: [ moment().add(1, 'days').startOf('day'), moment().add(1, 'year').endOf('day') ]
} as any;
...
export function calc2ClockDate(date?: string) {
    if (date === undefined || date === null || date === '') {
        ...
        const today2Clock = currentDay.clone().set({ hour: 2, minute: 0, second: 0 });
...
export function setStartTimeAndFormat(date: Moment) {
    const formatString = date.set({ hour: 0, minute: 0, second: 0 }).format('YYYY-MM-DD HH:mm:ss');
    return formatString;
}
...
export function setEndTimeAndFormat(date: Moment) {
    const formatString = date.set({ hour: 23, minute: 59, second: 59 }).format('YYYY-MM-DD HH:mm:ss');
    return formatString;
}
...
export function setUtcStartTimeAndFormat(date: Moment) {
    const formatString = date.clone().set({ hour: 0, minute: 0, second: 0 }).utc().format();
    return formatString;
}
...
export function setUtcEndTimeAndFormat(date: Moment) {
    const formatString = date.clone().set({ hour: 23, minute: 59, second: 59 }).utc().format();
    return formatString;
}
